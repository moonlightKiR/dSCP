...
class BalancedFaceDataset(Dataset):
    def __init__(self, illinois_csv, lfw_csv, transform=None):
        ...
        df_ill = pd.read_csv(
            illinois_csv, sep=";", on_bad_lines="skip", engine="python"
        )
        ...
        if min_size == 0:
            print(
                "ADVERTENCIA: Uno de los subgrupos tiene 0 muestras. El balanceo fallará."
            )
            # Intentamos un balanceo mínimo si es posible, o lanzamos error claro
            if sum(counts) == 0:
                raise ValueError(
                    "No se encontraron imágenes válidas para ninguna clase."
                )

        print(
            f"Balanceando dataset a {min_size} muestras por subgrupo (Total: {min_size * 4})"
        )

        g1 = df_ill[df_ill["race"] == "white"].sample(
            n=min_size, random_state=42
        )
        g2 = df_ill[df_ill["race"] == "black"].sample(
            n=min_size, random_state=42
        )
        g3 = df_lfw[df_lfw["race"] == "white"].sample(
            n=min_size, random_state=42
        )
        g4 = df_lfw[df_lfw["race"] == "black"].sample(
            n=min_size, random_state=42
        )

        g1["path"] = g1["filename"].apply(
            lambda x: os.path.join(PROCESSED_ILL_PATH, x)
        )
        g2["path"] = g2["filename"].apply(
            lambda x: os.path.join(PROCESSED_ILL_PATH, x)
        )
        g3["path"] = g3["filename"].apply(
            lambda x: os.path.join(PROCESSED_LFW_PATH, x)
        )
        g4["path"] = g4["filename"].apply(
            lambda x: os.path.join(PROCESSED_LFW_PATH, x)
        )
        ...
        self.data = (
            pd.concat([g1, g2, g3, g4]).sample(frac=1).reset_index(drop=True)
        )
