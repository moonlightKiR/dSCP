...
class DataPreprocessor:
    def __init__(self):
        ...
        self.cpu_detector = MTCNN(image_size=224, margin=30, post_process=False, device=torch.device("cpu"))
    ...
    def process_single_image_pil(self, img_pil, output_path):
        ...
        try:
            ...
            if face_tensor is not None:
                ...
                face_img = Image.fromarray(face_tensor.permute(1, 2, 0).cpu().numpy().astype('uint8'))
    ...
    def run_full_preprocessing(self, illinois_limit=500, lfw_limit=500, batch_size=32):
            ...
            for p in tqdm(files_to_process):
                ...
                if not os.path.exists(out_path):
                    try:
                        ...
                        continue
            print(f" Finalizado {config['name']}: {saved_count} imágenes nuevas guardadas en {config['dst']}")
