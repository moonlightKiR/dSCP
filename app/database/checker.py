...
class Checker:
    ...
    def _is_structure_correct(self, dataset_path: Path, expected_min_count: int) -> bool:
    ...
    def full_check(self):
        ...
        if not self._is_structure_correct(self.path_illinois, 0):
            print(f"Error: No se encontró la carpeta {self.path_illinois}. Se requiere descarga.")
        ...
        if not self._is_structure_correct(self.path_lfw, 0):
            print(f"Error: No se encontró la carpeta {self.path_lfw}. Se requiere descarga.")
            database = KaggleRetreiver()
            reconstructor = Reconstructor()
