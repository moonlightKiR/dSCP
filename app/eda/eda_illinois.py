...
class IllinoisEDA(EDABase):
    ...
    def analyze_quality(self, sample_size=500):
        ...
        print(
            f"Analyzing quality for {min(sample_size, len(self.front_images))} images..."
        )
        brightness_list = []
        contrast_list = []

        sample = random.sample(
            self.front_images, min(sample_size, len(self.front_images))
        )
    ...
    def analyze_emotions(self, sample_size=50):
        ...
        print(
            f"Analyzing emotions for {min(sample_size, len(self.front_images))} images (DeepFace)..."
        )
        emotions_list = []
        sample = random.sample(
            self.front_images, min(sample_size, len(self.front_images))
    ...
    def plot_demographics(self):
        ...
        # 1. Race Distribution
        sns.countplot(
            data=self.df_metadata, x="race", ax=axes[0], palette="viridis"
        )
