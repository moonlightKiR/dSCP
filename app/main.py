...
def main():
    ...
    print("====================================================")
    print("   INICIANDO PIPELINE COMPLETO DE DETECCIÓN")
    print("====================================================\n")

    # [PASO 0] Verificación de integridad (Checker)
    print("[PASO 0] Verificando datos originales...")
    checker = Checker()
    checker.full_check()

    # [PASO 1] EDA (Análisis Exploratorio de Datos)
    print("\n[PASO 1] Ejecutando EDA sobre imágenes originales...")
    # Pasamos las rutas base. El EDA de Illinois busca el CSV en ILLINOIS_CSV_PATH
    ...
    if not os.path.exists(LFW_CSV_PATH) or os.path.getsize(LFW_CSV_PATH) == 0:
        print(">> Generando metadatos étnicos de LFW...")
        lfw_eda.generate_ethnicity_csv(LFW_CSV_PATH)
    else:
        print(">> Metadatos de LFW detectados.")
    
    # [PASO 2] Preprocesamiento (Extracción Facial MTCNN)
    print("\n[PASO 2] Verificando extracción facial (MTCNN)...")
    if not os.path.exists(PROCESSED_ILL_PATH) or len(os.listdir(PROCESSED_ILL_PATH)) < 100:
        print(">> Iniciando preprocesamiento MTCNN (esto puede tardar)...")
        prep = DataPreprocessor()
        prep.run_full_preprocessing()
    else:
        ill_count = len(os.listdir(PROCESSED_ILL_PATH))
        lfw_count = len(os.listdir(PROCESSED_LFW_PATH))
        print(f">> Dataset procesado detectado ({ill_count} Illinois, {lfw_count} LFW).")

    # [PASO 3, 4 y 5] Entrenamiento y Evaluación de Modelos
    modelos_a_entrenar = ["resnet", "vgg"]

    for tipo in modelos_a_entrenar:
        nombre_bonito = "ResNet50" if tipo == "resnet" else "VGG-16"
        print(f"\n\n" + "#"*60)
        print(f"### EJECUTANDO EXPERIMENTO: {nombre_bonito} ###")
        print("#"*60)

        # [PASO 3] Entrenamiento y Auditoría (Matriz de Confusión)
        model, val_ds, device = run_full_experiment(model_type=tipo, epochs=3)

        # [PASO 4] Explicabilidad (Mapas de calor)
