...
from models.train_resnet import (
    run_full_experiment,
    visual_explanation,
    classify_from_url,
)
...
def main():
    ...
    ill_procesadas = (
        len(os.listdir(PROCESSED_ILL_PATH))
        if os.path.exists(PROCESSED_ILL_PATH)
        else 0
    )
    lfw_procesadas = (
        len(os.listdir(PROCESSED_LFW_PATH))
        if os.path.exists(PROCESSED_LFW_PATH)
        else 0
    )

    print(
        f"-> Conteo actual: {ill_procesadas} fotos en Illinois, {lfw_procesadas} fotos en LFW."
    )

    # Lógica de salto (si hay suficientes)
    if ill_procesadas > 5000 and lfw_procesadas > 5000:
        print(
            f">> Dataset procesado detectado ({ill_procesadas} Illinois, {lfw_procesadas} LFW)."
        )
        print(">> Saltando extracción facial para ahorrar tiempo...")
    else:
        print(
            f">>Faltan imágenes o el dataset está incompleto. Iniciando MTCNN masivo..."
        )
    ...
    print(
        "   Revisa la carpeta /reports para ver los resultados comparativos."
    )
    print("=" * 60)
